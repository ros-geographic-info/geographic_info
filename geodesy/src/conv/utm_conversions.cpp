/*
 *  Conversions between WGS 84 Latitude/Longitude and UTM
...
/**  @file
     ...
     Functions to convert (ellipsoidal) latitude and longitude to and
...
UTMPoint::UTMPoint(const geographic_msgs::GeoPoint &pt)
{
  ...
  this->zone = int((LongTemp + 180)/6) + 1;
  ...
  if( Lat >= 72.0 && Lat < 84.0 ) 
    {
      if(      LongTemp >= 0.0  && LongTemp <  9.0 ) this->zone = 31;
      else if( LongTemp >= 9.0  && LongTemp < 21.0 ) this->zone = 33;
      else if( LongTemp >= 21.0 && LongTemp < 33.0 ) this->zone = 35;
      else if( LongTemp >= 33.0 && LongTemp < 42.0 ) this->zone = 37;
    }
  ...
  this->band = UTMLetterDesignator(Lat);
#if 0
  if (this->band == ' ')
